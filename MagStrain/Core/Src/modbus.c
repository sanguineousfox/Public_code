...
static const Param_Float_Default default_float_params[] = {
    ...
    {MB_ADDR_PROBE_DEPTH, 0.0f}, {MB_ADDR_TANK_HEIGHT, 0.95f}, {MB_ADDR_DAMPING_TIME, 10.0f},
    {MB_ADDR_POLL_PERIOD, 1.0f}, {MB_ADDR_LEVEL_OFFSET, 0.0f}, {MB_ADDR_THRESH_LVL, 0.9f},
};
...
static void EEPROM_Initialize(void)
{
    ...
    if (!AT24C64_IsFormatted()) {
        ...
        AT24C64_LoadAllRegisters(modbus.regs, MODBUS_REG_ARRAY_SIZE);
    }
}
...
void ModBus_Init(void)
{
    ...
    for (int i = 0; i < DEFAULT_FLOAT_PARAMS_COUNT; i++) {
        ...
        if (idx != 0xFFFF) {
            ...
            if (cur == 0.0f || cur != cur || cur < -1e9f || cur > 1e9f)
