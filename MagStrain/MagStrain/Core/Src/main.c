...
/*
  @file           : main.c
  @brief          : Основной цикл (ОПТИМИЗИРОВАННЫЙ - без USART2)
  @description    : - Управление коммутацией на PB7
                    - Статистика: 11 значений, отброс мин/макс
                    - Мёртвое время: 65 тиков
                    - Modbus RTU @ 19200 бод
                    - БЕЗ вывода на USART2
                    - PB7 HIGH при КАЖДОМ захвате импульса
*/
...
/* === Мёртвое время и количество магнитов === */
#define BLANKING_WINDOW_TICKS   65
#define MAX_PULSE_PAIRS         2
/* MAX_CAPTURED_PULSES уже определен в main.h - не переопределяем! */

/* === Адреса Modbus регистров === */
#define MB_ADDR_POLL_PERIOD     5
#define MB_ADDR_CAL_LOW_LVL     7
#define MB_ADDR_CAL_HIGH_LVL    8

/* ==========================================================================
   ПРОТОТИПЫ ФУНКЦИЙ MODBUS (если их нет в modbus.h)
========================================================================== */
extern float ModBus_GetParameter_Float(uint8_t addr);
extern void  ModBus_SetParameter_Float(uint8_t addr, float value);
extern float ModBus_GetWaveguideLength(void);
extern void  ModBus_UpdateFirmwareVersion(uint16_t version);
...
static uint32_t Read_ADC_Average(ADC_HandleTypeDef *hadc, uint32_t channel, uint32_t sampling_time, uint8_t samples);
static uint32_t Read_ADC_Single(ADC_HandleTypeDef *hadc, uint32_t channel, uint32_t sampling_time);
...
/* ==========================================================================
   ...
   ИСПРАВЛЕНО: правильные сигнатуры функций Modbus из main(1).c
========================================================================== */
void Process_Measurement_Results(float tof_us, float position_mm, uint8_t signal_captured)
{
    ...
    if (cal_low < 0.1f || cal_low > 100.0f) cal_low = 0.1f;
    if (cal_high < 1.0f || cal_high > 50000.0f) cal_high = 3.0f;
    if (position_mm < 0.0f) position_mm = 0.0f;
    if (position_mm > waveguide_len) position_mm = waveguide_len;

    // ИСПРАВЛЕНО: 5 аргументов (как в main(1).c)
    ModBus_UpdateMeasurements(position_mm, current_temperature,
                              tof_error, temp_error, signal_captured);

    // ИСПРАВЛЕНО: 8 аргументов (как в main(1).c)
    ModBus_UpdateVoltages(current_vdda, current_24v, current_12v, current_5v,
                          v24_error || v12_error || v5_error,
                          v24_error, v12_error, v5_error);
...
int main(void)
{
    ...
    while (1)
    {
        // Мигание синим светодиодом (1 Гц)
            ...
            if (measurement > 0 && !tof_timeout) {
                ...
                if (stat_ready) {
                    ...
                    if (avg_ticks > 0.0f) {
                        ...
                        // Успешное измерение - зажигаем красный светодиод
...
void generate_pulse_and_measure(void)
{
    ...
    // PB7 LOW перед генерацией импульса
    HAL_GPIO_WritePin(SWITCH_PORT, SWITCH_PIN, GPIO_PIN_RESET);

    // Генерация импульса на PB5
...
uint32_t measure_time_of_flight(void)
{
    ...
    // Ждём окончания мёртвого времени
    do {
        cnt_value = TIM3->CNT;
    ...
    // PB7 LOW после завершения измерения
...
void MX_GPIO_Init(void)
{
    ...
    // PB7 (SWITCH_PIN - управление коммутацией)
